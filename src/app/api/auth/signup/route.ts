...
export const POST = async (request: NextRequest) => {
  ...
  try {
    ...
    try {
      ...
      if (error instanceof ZodError) { //if error is a ZodError
        const errmsg = error.flatten().fieldErrors; // codensing the error
        ...
        if (Object.keys(errmsg).length === 0) {
          return NextResponse.json({
            ...
            status: 400,
          }); // returning the eror for confirmpassword
        }
        return NextResponse.json({ message: firstErrorValue, status: 400 }); // displaying any other error message
      }
    }

    if (!requestbody.name || !requestbody.email || !requestbody.password || !requestbody.confirmPassword ) {
      // checking if any of these proprties name, email and id are not present using OR operator
      return NextResponse.json({
        // returning an error response
        message: "Name, email, password and confirmation not provided",
        status: 400,
      });
    }
    ...
    const newUser: User = {
      //creating a new user to be added to the database
